...
function listCitys() {
	...
	for (var i = 0; i < citys.length; i++) {
		...
		if (find == "" || city.indexOf(find) != -1) {
			...
			for (var j = 0; j < countrys.dataArray.length; j++) {
				if (countrys.dataArray[j].idCountry == citys[i].idCountry) {
					data += '<td>' + countrys.dataArray[j].name + '</td>';
					break;
				}
			}
	...
	if (data == "") {
		$('#msfind').html("No se encontraron ciudades");
	} else { $('#msfind').html(""); }
	content += data + '</table>';
	$('#lista').html(content);
}
